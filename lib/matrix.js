...
function _dot (a, b) {
  ...
  if(!isMatrix(b)) {
    // We can do matrix . vector
    if(isMatrix(a)) {
      // Only if vector.length = matrix.columns
      if(vector.dimensions(b) != adims[1]) { throw new Error('Dot product requires matrix.columns = vector.dimensions') }
    ...
    // But we can't do vector . matrix
      ...
      throw new Error('Dot products of the form (vector . matrix) are not possible')
    }
  }

  if(adims[1] != bdims[0]) { throw new Error('Dot product requires two matrices a, b where a.columns = b.rows') }
...
function dot () {
  return Array.prototype.slice.call(arguments).reduce(_dot)
}
